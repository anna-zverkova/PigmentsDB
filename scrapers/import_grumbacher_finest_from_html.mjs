#!/usr/bin/env node
/**
 * Import Grumbacher Finest watercolor paints from a saved Dr. Oto Kano HTML page.
 *
 * - Parses the paint table from the archived HTML
 * - Matches swatches only against public/uploads/grumbacher-finest
 * - Leaves swatchImage blank when the local swatch file does not exist
 * - Appends new paint rows to content/paints.json
 *
 * Usage:
 *   node scrapers/import_grumbacher_finest_from_html.mjs \
 *     --html "Grumbacher Finest - Watercolor Paint Database- Dr. Oto Kano.html" \
...
const DEFAULT_HTML = 'Grumbacher Finest - Watercolor Paint Database- Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/grumbacher-finest';
const DEFAULT_BRAND_ID = 'grumbacher-finest';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the Grumbacher Finest paint table.');
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const swatchFilename = swatchInfo.filename || `gf-${slugify(name)}.png`;
    const localSwatchFilename = uploadByLowerName.get(swatchFilename.toLowerCase()) || '';
    const localSwatchPath = localSwatchFilename ? `/uploads/grumbacher-finest/${localSwatchFilename}` : '';
  ...
  console.log(`Processed ${rows.length} Grumbacher Finest rows`);
  console.log(`Local swatches checked in ${uploadsDir}`);
  console.log(`Existing Grumbacher Finest rows updated: ${seenNames.size - added}`);
