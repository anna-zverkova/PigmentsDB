#!/usr/bin/env node
/**
 * Import Utrecht watercolor paints from a saved Dr. Oto Kano HTML page.
 *
...
const DEFAULT_HTML = 'Utrecht - Paint Database - Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/utrecht';
const DEFAULT_BRAND_ID = 'utrecht';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the Utrecht paint table.');
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const localSwatchPath = localSwatchFilename ? `/uploads/utrecht/${localSwatchFilename}` : '';
    ...
    const veganValue = stripTags(cells[12]).toUpperCase();
    ...
    paints.push({
      ...
      granulation: null,
      lightfastness: null,
      isVegan: veganValue === 'V' ? true : veganValue === 'NO' ? false : null,
  ...
  console.log(`Processed ${rows.length} Utrecht rows`);
  console.log(`Local swatches checked in ${uploadsDir}`);
  console.log(`Existing Utrecht rows updated: ${seenNames.size - added}`);
