#!/usr/bin/env node
/**
 * Import Daler Rowney Professional watercolor paints from a saved Dr. Oto Kano HTML page.
 *
 * - Parses the paint table from the archived HTML
 * - Matches swatches by filename from public/uploads/daler-rowney-pro
...
const DEFAULT_HTML = 'Daler Rowney Pro - Watercolor Paint Database - Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/daler-rowney-pro';
const DEFAULT_BRAND_ID = 'daler-rowney-prof';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the Daler Rowney Pro paint table.');
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const localSwatchPath = `/uploads/daler-rowney-pro/${swatchFilename}`;
  ...
  console.log(`Processed ${rows.length} Daler Rowney Pro rows`);
  console.log(`Swatches ensured in ${uploadsDir}`);
  console.log(`Existing Daler Rowney Pro rows updated: ${seenNames.size - added}`);
