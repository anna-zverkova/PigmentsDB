#!/usr/bin/env node
/**
 * Import Blick watercolor paints from a saved Dr. Oto Kano HTML page.
 *
...
const DEFAULT_HTML = 'Blick Watercolors - Watercolor Paint Database - Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/blick';
const DEFAULT_BRAND_ID = 'blick';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the Blick paint table.');
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const localSwatchPath = localSwatchFilename ? `/uploads/blick/${localSwatchFilename}` : '';
    ...
    paints.push({
      ...
      pigmentCodes,
      hex: '#E6D9C6',
      transparency: toMaybeDash(cells[11]),
      staining: toMaybeDash(cells[9]),
      granulation: toMaybeDash(cells[10]),
      lightfastness: toMaybeDash(cells[8]),
      isVegan: stripTags(cells[12]).toLowerCase() === 'v' ? true : null,
  ...
  console.log(`Processed ${rows.length} Blick rows`);
  console.log(`Local swatches checked in ${uploadsDir}`);
  console.log(`Existing Blick rows updated: ${seenNames.size - added}`);
