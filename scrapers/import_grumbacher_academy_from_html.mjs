#!/usr/bin/env node
/**
 * Import Grumbacher Academy watercolor paints from a saved Dr. Oto Kano HTML page.
 *
...
const DEFAULT_HTML = 'Grumbacher Academy - Watercolor Database - Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/grumbacher-academy';
const DEFAULT_BRAND_ID = 'grumbacher-academy';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the Grumbacher Academy paint table.');
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const candidates = [
      imageInfo.filename,
      `ga-${slugify(name)}.png`,
      `ga-${slugify(name.replace(/\s*\([^)]*\)\s*/g, ' ').trim())}.png`,
    ].filter(Boolean);
    const localSwatchFilename = candidates.map((candidate) => uploadByLowerName.get(candidate.toLowerCase()) || '').find(Boolean) || '';
    const localSwatchPath = localSwatchFilename ? `/uploads/grumbacher-academy/${localSwatchFilename}` : '';
    ...
    paints.push({
      ...
      staining: toStaining(cells[9]),
      granulation: toGranulation(cells[10]),
      lightfastness: toLightfastness(cells[8]),
      isVegan: vegan === 'v' ? true : vegan === 'no' ? false : null,
  ...
  console.log(`Processed ${rows.length} Grumbacher Academy rows`);
  console.log(`Local swatches checked in ${uploadsDir}`);
  console.log(`Existing Grumbacher Academy rows updated: ${seenNames.size - added}`);
