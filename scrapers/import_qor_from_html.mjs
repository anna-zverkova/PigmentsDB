#!/usr/bin/env node
/**
 * Import QoR watercolor paints from a saved Dr. Oto Kano HTML page.
 *
...
const DEFAULT_HTML = 'QoR - Watercolor Paints Database - Dr. Oto Kano.html';
const DEFAULT_PAINTS = 'content/paints.json';
const DEFAULT_UPLOADS_DIR = 'public/uploads/qor';
const DEFAULT_BRAND_ID = 'qor';
...
function parseTableRows(html) {
  ...
  if (!table) throw new Error('Could not find the QoR paint table.');
...
function parseLightfastness(value) {
...
async function main() {
  ...
  for (const cells of rows) {
    ...
    const localSwatchPath = localSwatchFilename ? `/uploads/qor/${localSwatchFilename}` : '';
    ...
    paints.push({
      ...
      lightfastness: parseLightfastness(cells[8]),
      isVegan: stripTags(cells[12]).toUpperCase() === 'V' ? true : stripTags(cells[12]).toUpperCase() === 'NO' ? false : null,
  ...
  console.log(`Processed ${rows.length} QoR rows`);
  console.log(`Local swatches checked in ${uploadsDir}`);
  console.log(`Existing QoR rows updated: ${seenNames.size - added}`);
